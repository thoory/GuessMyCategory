...
public class SessionResourceIT {
    ...
    public void getAllSessionsWithEagerRelationshipsIsEnabled() throws Exception {
        SessionResource sessionResource = new SessionResource(sessionRepositoryMock);
        when(sessionRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSessionMockMvc.perform(get("/api/sessions?eagerload=true"))
            .andExpect(status().isOk());
    ...
    @SuppressWarnings({"unchecked"})
    public void getAllSessionsWithEagerRelationshipsIsNotEnabled() throws Exception {
        SessionResource sessionResource = new SessionResource(sessionRepositoryMock);
        when(sessionRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
